package com.omkaaij.omkaaij_nl.exception;
...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    ResponseEntity<Object> handleConstraintViolationException(ConstraintViolationException e) {
        List<ConstraintViolation<?>> constraintViolationList = new ArrayList<>(e.getConstraintViolations());
        ConstraintViolation<?> constraintViolation = constraintViolationList.get(0);
//        String constraintViolationParameter = (PathImpl) constraintViolation.getPropertyPath().getLeafNode().asString();
        String constraintViolationMessage = constraintViolation.getMessage();
        String errorMessage = String.format("Parameter '%s'", constraintViolationMessage);
